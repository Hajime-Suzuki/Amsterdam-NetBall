...
class MemberCommitteesModal extends PureComponent {
  ...
  render() {
    const { committeesModalOpen, handleClose, getModalStyle, classes, committees, member } = this.props
    const memberCommitteeIds = member.committees.map( committee => committee.id )
    return (<Modal
          ...
          <ul className="committee-assignments">
            {
              committees.map(committee => {
                const memberStatus = memberCommitteeIds.includes(committee.id) ? "on-committee" : "not-on-committee"
                return (<li key={committee.id} onClick={ ()=>this.toggleMemberCommittee(committee, member) } className={memberStatus}>
                  <span className="committee-name">{ committee.name }</span>
                  { memberCommitteeIds.includes(committee.id) ? <span className="committee-check">&#10004;</span> : '' }
                </li>)
              }
              )
            }
